...
public class QueuedEventConsumer implements EventConsumer {
  ...
  private static final String DISPATCHER_THREAD_NAME = "styx-event-handler-dispatcher";
  private static final int SHUTDOWN_GRACE_PERIOD_SECONDS = 5;

  private final Thread dispatcherThread;
  private final Object signal = new Object();
  private final EventInterceptor eventReceiver;
  ...
  public QueuedEventConsumer(EventInterceptor eventReceiver) {
    this.eventReceiver = Objects.requireNonNull(eventReceiver);
  ...
  private void dispatch() {
    while (running || queue.size() > 0) {
      if (queue.size() > 0) {
        try {
          eventReceiver.eventTransitioned(queue.remove());
