...
class DatastoreStorage {
  ...
  private Backfill entityToBackfill(Entity entity) {
    ...
    if (entity.contains(PROPERTY_DOCKER_IMAGE)) {
      dockerImage = Optional.of(entity.getString(PROPERTY_DOCKER_IMAGE));
    } else {
      dockerImage = Optional.empty();
    }

    return Backfill.create(
        entity.key().name(),
        datetimeToInstant(entity.getDateTime(PROPERTY_START)),
        datetimeToInstant(entity.getDateTime(PROPERTY_END)),
        entity.getString(PROPERTY_COMPONENT),
        entity.getString(PROPERTY_WORKFLOW),
        entity.getLong(PROPERTY_CONCURRENCY),
        dockerImage,
        entity.getBoolean(PROPERTY_PROCESSED),
        entity.getString(PROPERTY_RESOURCE));
  }
  ...
  private Entity backfillToEntity(Backfill backfill) {
    ...
    Entity.Builder builder = Entity.builder(key)
        ...
        .set(PROPERTY_COMPONENT, backfill.component())
        .set(PROPERTY_WORKFLOW, backfill.workflow())
        .set(PROPERTY_PROCESSED, backfill.processed())
        .set(PROPERTY_RESOURCE, backfill.resource());

    if (backfill.dockerImage().isPresent()) {
      builder.set(PROPERTY_DOCKER_IMAGE, backfill.dockerImage().get());
    }
